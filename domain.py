...
class Parcel:
    ...
    ==== Public Attributes ====
    id: unique identifier for any given parcel. No two ids are the same
    volume: volume of the parcel, measured in cubic centimeters
    source: name of the city which the parcel came from
    destination: name of the city which the parcel is going to
    """
    id: int
    volume: int
    source: str
    ...
    def __str__(self) -> str:
        """Return a string representation of a parcel"""
        return "A parcel of size {0}, travelling from {1} to " \
               "{2}.".format(self.volume, self.source, self.destination)
...
class Truck:
    ...
    ==== Public Attributes ===
    id: unique identifier of the truck. No two ids are the same
    capacity: the maximum volume a truck can carry, measured in
    cubic centimeters
    source: name of city in which came from
    load: current carrying load in cubic centimeters
    parcels: a list of all parcels being carried by the truck
    """
    id: int
    capacity: int
    source: str
    ...
    def __init__(self, i: int, c: int, s: str) -> None:
    ...
    def __str__(self) -> str:
        """Return a string representation of a truck and its parcels"""
        if self.parcels == []:
            return "An empty truck of capacity {0} " \
                   "from {1}".format(self.capacity, self.source)
        result = "A truck with carrying capacity {0} of {1} from {2} " \
                 "with the following parcels: \n".format(self.load,
                                                         self.capacity,
                                                         self.source)
    ...
    def pack(self, p: Parcel) -> bool:
        """Pack parcel p into the truck if p can fit. Return True if p is
        packed successfully, or False if not
        >>> p = Parcel(1, 5, 'Toronto', 'Peel')
        ...
        False
        """
        if p.volume <= self.capacity - self.load:
            self.load += p.volume
    ...
    def fullness(self) -> float:
        """Return percentage fullness of truck carrying capacity
        >>> p = Parcel(1, 5, 'Toronto', 'Peel')
        >>> t = Truck(1000, 10, 'Toronto')
        >>> t.fullness()
        0.0
...
class Fleet:
    ...
    def __str__(self) -> str:
        ...
        """
        result = "A fleet of the following trucks: \n"
